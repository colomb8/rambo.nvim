...
Author: Dario Colombotto dario.colombotto@outlook.com

Vision:
 - Normal Mode: pure Vim experience, no arrow keys
 - Insert Mode: behaves like Sublime, Notepad++, etc.

Why this choice?
 - it's a pity to overturn the usage of Normal Mode and leaving it as is
 allows the user to mantain his proficiency in "pure Vim";
 - The Normal mode in where the great part of magic keybindings are used
 so use arrows there is, in conclusion, inefficient.
 - On other hand, Insert mode is poor of keybindings *compared to normal*.

-- C right and left usano vim.opt.keyword

-- Important Note: both inclusive and exclusive selection are supported.
-- all edge cases have been handled and this took a lot of effort
-- in an off by one nightmare.
-- each motion has been carefully designed and tested.
-- to be more usable also with non US keyboards
-- promote the use of vim not only for coding
-- avvicinare piu' persone all'uso di questo potente editor
...
local function byteToUtf(line, col)
  utf_32_index, utf_16_index = vim.str_utfindex(line, col and col - 1)
  return utf_32_index
end
...
assert(
  vim.list_contains({'C', 'M'}, operations_key),
  'operation_key supports only "C" and "M"; received ' .. operations_key)
